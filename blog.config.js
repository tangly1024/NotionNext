...
const BLOG = {
  ...
  THEME: process.env.NEXT_PUBLIC_THEME || 'hexo', // 当前主题，在themes文件夹下可找到所有支持的主题；主题名称就是文件夹名，例如 example,fukasawa❎,gitbook✅,heo❎,hexo✅,landing,matery,medium,next,nobelium,plog,simple,nav✅,starter
  ...
  NOTION_HOST: process.env.NEXT_PUBLIC_NOTION_HOST || 'https://zhouzhousblog.online', // Notion域名，您可以选择用自己的域名进行反向代理，如果不懂得什么是反向代理，请勿修改此项https://www.notion.so
  ...
  RANDOM_IMAGE_URL: process.env.NEXT_PUBLIC_RANDOM_IMAGE_URL || '', // 随机图片API,如果未配置下面的关键字，主页封面，头像，文章封面图都会被替换为随机图片https://api.yimian.xyz/img
  RANDOM_IMAGE_REPLACE_TEXT:
    process.env.NEXT_PUBLIC_RANDOM_IMAGE_NOT_REPLACE_TEXT ||
    'images.unsplash.com', // 触发替换图片的 url 关键字(多个支持用英文逗号分开)，只有图片地址中包含此关键字才会替换为上方随机图片url
  // eg: images.unsplash.com(notion图床的所有图片都会替换),如果你在 notion 里已经添加了一个随机图片 url，恰巧那个服务跑路或者挂掉，想一键切换所有配图可以将该 url 配置在这里https://bing.img.run/rand.php
  ...
  POST_WAITING_TIME_FOR_404:
    process.env.NEXT_PUBLIC_POST_WAITING_TIME_FOR_404 || '16', // 文章加载超时时间，单位秒；超时后跳转到404页面
  ...
  WIDGET_PET_LINK:
    ...
    'https://cdn.jsdelivr.net/npm/live2d-widget-model-ni-j/assets/ni-j.model.json', // 挂件模型地址 @see https://github.com/xiazeyu/live2d-widget-models  https://cdn.jsdelivr.net/npm/live2d-widget-model-wanko@1.0.5/assets/wanko.model.json
  ...
  HOME_BANNER_IMAGE:
    process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE || '/bg_image1.jpg', // 首页背景大图, 会被notion中的封面图覆盖，若无封面图则会使用代码中的 /public/bg_image.jpg 文件
  DESCRIPTION:
    process.env.NEXT_PUBLIC_DESCRIPTION || '希望对你有所帮助', // 站点描述，被notion中的页面描述覆盖
