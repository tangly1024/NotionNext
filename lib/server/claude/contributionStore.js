...
const buildEventId = (type, repositoryId, timestampMs) => {
  return `sync:${type}:${repositoryId}:${timestampMs}`
}
...
export const buildBodySnapshotFromBlockMap = (pageId, blockMap) => {
  const blocks = blockMap?.block || {}
  const allBlockIds = Object.keys(blocks)
  if (!allBlockIds.length) {
    return { contentFingerprint: '', contentEditedAtMs: 0 }
  }

  const normalizedPageId = normalizeRepositoryId(pageId)
  const pageBlock = allBlockIds
    .map(blockId => blocks[blockId]?.value)
    .find(value => {
      return value?.type === 'page' && normalizeRepositoryId(value?.id) === normalizedPageId
    })

  const rootContent = Array.isArray(pageBlock?.content) ? pageBlock.content : []
  ...
  const walk = blockId => {
    ...
    const blockType = normalizeText(value.type || 'unknown')
    const editedAtMs = toTimestampMs(value.last_edited_time)
    const childIds = Array.isArray(value.content) ? value.content : []

    tokens.push(`${blockId}:${blockType}:${editedAtMs}:${childIds.length}`)
    if (editedAtMs > contentEditedAtMs) contentEditedAtMs = editedAtMs
  ...
  return {
    contentFingerprint: tokens.length ? md5(tokens.join('|')) : '',
    contentEditedAtMs
  }
}

export const buildContributionPostSnapshot = (post, blockMap) => {
  ...
  const fallbackUpdatedAtMs = Math.max(toTimestampMs(post?.lastEditedDate), createdAtMs)
  const bodySnapshot = buildBodySnapshotFromBlockMap(post?.id, blockMap)
  const updatedAtMs = Math.max(bodySnapshot.contentEditedAtMs, fallbackUpdatedAtMs)
  ...
  return {
    ...
    createdAtMs,
    updatedAtMs,
    contentFingerprint: normalizeText(bodySnapshot.contentFingerprint),
    source: 'notion_post_snapshot'
  }
}
...
const normalizeRawEvent = raw => {
  ...
  const eventId = candidateEventId || [
    ...
    repositoryId,
    timestampMs,
    md5(`${normalizeText(raw.title)}|${normalizeText(raw.slug)}`)
  ].join(':')
  ...
  return {
    ...
    source: normalizeText(raw.source || 'manual_import')
  }
}
...
const loadSnapshotMap = async (client, repositoryIds) => {
  ...
  for (const chunk of chunkArray(uniqueIds, 200)) {
    const { data, error } = await client
      .from(SNAPSHOTS_TABLE)
      .select(
        'repository_id, title, slug, created_at_ms, updated_at_ms, content_fingerprint, synced_at_ms'
      )
    ...
    ;(data || []).forEach(row => {
      ...
      map.set(repositoryId, {
        ...
        contentFingerprint: normalizeText(row.content_fingerprint),
...
export const syncContributionSnapshots = async postSnapshots => {
  ...
  const normalizedSnapshots = snapshots
    .map(snapshot => {
      ...
      return {
        ...
        updatedAtMs,
        contentFingerprint: normalizeText(snapshot.contentFingerprint)
      }
    })
    .filter(Boolean)
  ...
  normalizedSnapshots.forEach(snapshot => {
    ...
    const previousFingerprint = normalizeText(prev.contentFingerprint)
    const hasCurrentFingerprint = Boolean(snapshot.contentFingerprint)
    const hasPreviousFingerprint = Boolean(previousFingerprint)

    let shouldAppendUpdate = false
    if (hasCurrentFingerprint && hasPreviousFingerprint) {
      shouldAppendUpdate = snapshot.contentFingerprint !== previousFingerprint
    } else {
      shouldAppendUpdate = snapshot.updatedAtMs && snapshot.updatedAtMs > previousUpdatedAtMs
    }

    if (shouldAppendUpdate) {
      const updateTimestamp = Math.max(snapshot.updatedAtMs, previousUpdatedAtMs + 1)
      eventsToInsert.push({
...
export const listContributionEvents = async ({ limit = 50000 } = {}) => {
  ...
  const { data, error } = await client
    .from(EVENTS_TABLE)
    .select('event_id, event_type, repository_id, timestamp_ms, title, slug, source')
