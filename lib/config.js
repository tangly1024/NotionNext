...
if (/^\d+$/.test(val)) {
  ...
  // 【关键修正】对于扣子 Bot ID 或 PAT Token（以 pat_ 开头的），强制返回字符串
  // 或者如果数值超过最大安全整数，也返回字符串
  const isCozeRelated = val.startsWith('pat_') || val.length > 15 // 长度超过15的数字ID强制字符串
  if (parsedNum > Number.MAX_SAFE_INTEGER || isCozeRelated) {
    return val + ''
  }
  return parsedNum
}
