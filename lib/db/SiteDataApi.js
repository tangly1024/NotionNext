...
post = props?.allPages?.find(p => {
  ...
  const pageSlug = typeof p.slug === 'string' ? p.slug : ''
  const pageId = typeof p.id === 'string' ? p.id : ''
  const pageUuid = safeIdToUuid(pageId)

  return (
    slugCandidates.has(pageSlug) ||
    slugCandidates.has(pageSlug.split('/').pop()) ||
    (pageId && slugCandidates.has(pageId)) ||
