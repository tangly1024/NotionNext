...
// ─── 抽取：从 allPages 匹配 post ────────────────────────────────────────────

function findPostBySlug(allPages, fullSlug) {
...
function findPostById(allPages, fullSlug) {
  if (!fullSlug) return null
  return allPages?.find(p => p?.id === fullSlug) ?? null
}

async function findPostByUuid(lastSegment) {
  if (!/^[a-f0-9-]{32,36}$/i.test(lastSegment)) return null
  try {
    return await fetchPageFromNotion(lastSegment)
  } catch (e) {
...
async function fetchPostBlockMap(post, source) {
  if (!post?.id || post?.blockMap) return
  try {
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  const segments = [prefix, slug, ...(Array.isArray(suffix) ? suffix : [])].filter(Boolean)
  ...
  const post =
    findPostBySlug(props?.allPages, fullSlug) ||
    findPostById(props?.allPages, fullSlug) ||
    await findPostByUuid(lastSegment)

  await fetchPostBlockMap(post, source)
