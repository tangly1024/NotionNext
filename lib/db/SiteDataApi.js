...
export async function resolvePostProps({
  ...
  const normalizeSlugCanonical = input =>
    normalizeSlugPath(input)
      .toLowerCase()
      .replace(/[-_\s]/g, '')
  ...
  const normalizedFullSlug = normalizeSlugPath(fullSlug)
  ...
  if (fullSlug) {
    post = props?.allPages?.find(p => {
      ...
      return (
        p.slug === fullSlug ||
        normalizeSlugPath(p.slug) === normalizedFullSlug
      )
    })
  }

  /**
   * 3️⃣-1️⃣ 兼容匹配：忽略大小写与分隔符（-/_/空格）
   * 仅在精确匹配失败时生效，并且要求唯一结果，避免误命中
   */
  if (!post && fullSlug) {
    const targetCanonical = normalizeSlugCanonical(normalizedFullSlug)
    const fuzzyMatched =
      props?.allPages?.filter(p => {
        if (!p || p?.type?.includes('Menu')) return false
        return normalizeSlugCanonical(p.slug) === targetCanonical
      }) || []
