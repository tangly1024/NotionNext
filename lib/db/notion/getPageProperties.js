...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  const hasSlugPattern = allSlugPatterns.includes('%slug%')
  const slugValue = postProperties.slug ?? postProperties.id

  if (fullPrefix) {
    return hasSlugPattern ? fullPrefix : `${fullPrefix}/${slugValue}`
  } else {
    return `${slugValue}`
  }
}
