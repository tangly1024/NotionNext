...
/**
 * 将 Notion Decoration[] 格式的富文本数组转为 HTML
 */
function richTextToHtml(textArray) {
  if (!textArray || !Array.isArray(textArray)) return ''
  ...
  return textArray
    .map(item => {
      ...
      if (rawValue === '⁍') {
        const eq = decorations?.find(d => d[0] === 'e')
        return eq ? `<code>${escapeHtml(eq[1])}</code>` : ''
      }
      ...
      if (rawValue === '‣') {
        ...
        if (ref?.[0] === 'lm') {
          const href = ref[1]?.href || '#'
          const title = ref[1]?.title || href
          return `<a href="${escapeAttr(href)}">${escapeHtml(title)}</a>`
        }
        if (ref?.[0] === 'd') {
          return escapeHtml(ref[1]?.start_date || '')
        }
        return ''
      }

      let html = escapeHtml(rawValue)
      if (!decorations) return html
      ...
      for (const dec of decorations) {
        switch (dec[0]) {
          ...
          case '_':
            ...
            break
          case 'a':
            html = `<a href="${escapeAttr(dec[1])}">${html}</a>`
            break
          case 'h':
            break
          case 'e':
            html = `<code>${escapeHtml(dec[1])}</code>`
            break
          default:
            break
        }
...
function renderTable(blocks, block) {
  ...
  const hasHeadRow = toFlag(block.format?.table_block_column_header)
  const hasHeadCol = toFlag(block.format?.table_block_row_header)
  ...
  // Markdown 表格语法无法表达 row header（首列 th[scope=row]）；
  // 若开启了 row header 或关闭了 column header，改用语义化 HTML 表格输出。
  if (!hasHeadCol && hasHeadRow) {
  ...
  const renderCellsHtml = (row, options = {}) => {
    const { isHeaderRow = false, isBodyRow = false } = options
    const props = row.properties || {}
    return columnOrder
      .map((col, index) => {
        const raw = richTextToHtml(props[col]) || ''
        const content = raw.trim() ? raw : '&nbsp;'

        if (isHeaderRow) {
          return `<th scope="col">${content}</th>`
        }
        if (isBodyRow && hasHeadCol && index === 0) {
  ...
  const bodyStart = hasHeadRow ? 1 : 0
  const thead = hasHeadRow && rows[0]
    ? `<thead><tr>${renderCellsHtml(rows[0], { isHeaderRow: true })}</tr></thead>`
    : ''
  const tbodyRows = rows
    .slice(bodyStart)
    .map(row => `<tr>${renderCellsHtml(row, { isBodyRow: true })}</tr>`)
    .join('')
