...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      const rawPageData = await notionAPI.getPage(id)
      const pageData = rawPageData?.recordMap || rawPageData
      if (!pageData?.block) {
        throw new Error(
          `Invalid Notion page payload shape, missing "block". keys=${Object.keys(rawPageData || {}).join(',')}`
        )
      }
