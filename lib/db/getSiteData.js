...
export async function getGlobalData({
  ...
  return handleDataBeforeReturn(deepClone(data), locale)
}
...
function handleDataBeforeReturn(db, locale) {
  ...
  db.customMenu = sanitizeMenuItems(cleanIds(db?.customMenu), locale, db?.customNav)
...
function sanitizeMenuItems(items, locale, customNav = []) {
  if (locale === 'en-US') {
    return buildEnglishMenu(customNav)
  }
  ...
  return items
    .map(item => {
      ...
      if (Array.isArray(next.subMenus)) {
        next.subMenus = sanitizeMenuItems(next.subMenus, locale)
      }
...
function buildEnglishMenu(customNav = []) {
  const pageMap = new Map(
    (Array.isArray(customNav) ? customNav : [])
      .filter(item => item?.href && item?.name)
      .map(item => [item.href, item.name])
  )

  const getName = (href, fallback) => pageMap.get(href) || fallback

  return [
    { name: 'Home Page', href: '/', target: '_self' },
    { name: getName('/tools', 'Tools'), href: '/tools', target: '_self' },
    {
      name: 'Practical Insights',
      target: '_self',
      subMenus: [
        {
          name: getName('/prompt', 'Prompt'),
          href: '/prompt',
          target: '_self'
        },
        {
          name: getName('/learning', 'Learning'),
          href: '/learning',
          target: '_self'
        },
        {
          name: getName('/cases', 'Cases'),
          href: '/cases',
          target: '_self'
        }
      ]
    },
    {
      ...
      target: '_self',
      subMenus: [
        { name: 'AI News', href: '/category', target: '_self' }
      ]
    },
    {
      name: getName('/paper', 'Paper'),
      target: '_self',
      subMenus: [
        {
          name: getName('/paper', 'AI + Academic Papers'),
          href: '/paper',
          target: '_self'
        },
    ...
    { name: 'Search', href: '/search', target: '_self' }
  ]
}
