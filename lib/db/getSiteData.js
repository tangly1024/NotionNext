...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 修复：Notion API返回的数据有双层嵌套 block[pageId].value.value
  const blockItem = block[pageId]?.value
  const rawMetadata = blockItem?.value || blockItem

  // 调试信息
  console.log('[调试] pageId:', pageId)
  console.log('[调试] block对象中的键数量:', Object.keys(block).length)
  console.log('[调试] block中的所有键:', Object.keys(block))
  console.log('[调试] block中是否有pageId:', pageId in block)

  // 检查block中的第一个键的值
  const firstKey = Object.keys(block)[0]
  if (firstKey) {
    console.log('[调试] block的第一个键:', firstKey)
    console.log('[调试] block[firstKey]的类型:', typeof block[firstKey])
    console.log('[调试] block[firstKey]是否有value属性:', 'value' in (block[firstKey] || {}))
  }

  console.log('[调试] blockItem存在:', !!blockItem)
  console.log('[调试] rawMetadata存在:', !!rawMetadata)
  console.log('[调试] rawMetadata.type:', rawMetadata?.type)
  console.log('[调试] rawMetadata.collection_id:', rawMetadata?.collection_id)
  console.log('[调试] rawMetadata.view_ids:', rawMetadata?.view_ids?.length)

  // 如果rawMetadata是undefined，尝试直接从pageRecordMap中获取
  if (!rawMetadata && pageRecordMap) {
    console.log('[调试] pageRecordMap的顶级键:', Object.keys(pageRecordMap))
  }
  ...
  if (
    ...
    console.error(`实际类型: ${rawMetadata?.type || 'undefined'}`)
    return EmptyData(pageId)
  }
