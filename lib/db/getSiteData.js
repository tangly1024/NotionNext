  ...
  try {
    for (let index = 0; index < siteIds.length; index++) {
      ...
      if (index === 0 || locale === prefix) {
        const siteData = await getSiteDataByPageId({
          pageId: id,
          from
        })
        data = normalizeSiteData(siteData, id)
      }
    }
  } catch (error) {
...
function normalizeSiteData(siteData, pageId) {
  if (siteData && Array.isArray(siteData.allPages)) {
    return siteData
  }
...
function handleDataBeforeReturn(db) {
  ...
  db.allNavPages = ensurePageHref(db.allNavPages)
  db.allPages = ensurePageHref(db.allPages)
  db.latestPosts = ensurePageHref(db.latestPosts)
  ...
  return removeUndefinedFields(db)
}

/**
 * 深度移除对象中的 undefined，避免 Next.js getStaticProps 序列化报错
 * @param {*} value
 * @returns {*}
 */
function removeUndefinedFields(value) {
  ...
  if (value && typeof value === 'object') {
    ...
    Object.keys(value).forEach(key => {
      ...
      if (child !== undefined) {
        result[key] = removeUndefinedFields(child)
      }
    })
    return result
  }
...
/**
 * 为页面补齐 href，避免 next/link 因 undefined href 导致 SSR 崩溃
 * @param {Array} pages
 * @returns {Array}
 */
function ensurePageHref(pages) {
  ...
  return pages.map(page => {
    ...
    if (typeof page.href === 'string' && page.href.length > 0) {
      return page
    }
