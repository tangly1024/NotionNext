 ...
 */
const RESOURCE_STATUS_ATTR = 'data-notion-next-resource-status'
...
function findExistingResourceTag(url, type) {
  ...
  if (type === 'js') {
    ...
    for (const script of scripts) {
      if (toAbsoluteUrl(script.src) === absoluteUrl) {
        return script
      }
    }
    return null
  ...
  for (const link of links) {
    if (toAbsoluteUrl(link.href) === absoluteUrl) {
      return link
    }
  }
  return null
...
function waitForExistingResource(tag, url, type) {
  return new Promise((resolve, reject) => {
    const status = tag.getAttribute(RESOURCE_STATUS_ATTR)
    if (status === 'loaded') {
      resolve(url)
      return
    }
    ...
    // 非本工具创建的资源标签，沿用“已存在即复用”策略
    if (!tag.hasAttribute(RESOURCE_STATUS_ATTR)) {
      resolve(url)
      return
    }

    if (type === 'css' && tag.sheet) {
      tag.setAttribute(RESOURCE_STATUS_ATTR, 'loaded')
      resolve(url)
      return
    }
    ...
    const onError = () => {
      cleanup()
      tag.setAttribute(RESOURCE_STATUS_ATTR, 'error')
      reject(url)
    }
...
function createExternalResourceTag(url, type) {
  let tag
...
export function loadExternalResource(url, type = 'js') {
  ...
  const requestKey = `${type}:${toAbsoluteUrl(url)}`
  if (resourceLoadingMap.has(requestKey)) {
    return resourceLoadingMap.get(requestKey)
  }

  const loadPromise = new Promise((resolve, reject) => {
    const existingTag = findExistingResourceTag(url, type)
    if (existingTag) {
      const existingStatus = existingTag.getAttribute(RESOURCE_STATUS_ATTR)
      if (existingStatus === 'error') {
        existingTag.remove()
      } else {
        waitForExistingResource(existingTag, url, type)
          .then(resolve)
          .catch(reject)
        return
      }
    ...
    const tag = createExternalResourceTag(url, type)
    if (!tag) {
      resolve(url)
      return
    }

    tag.setAttribute(RESOURCE_STATUS_ATTR, 'loading')
  ...
  const trackedPromise = loadPromise.finally(() => {
    resourceLoadingMap.delete(requestKey)
  })

  resourceLoadingMap.set(requestKey, trackedPromise)
