...
export async function processPostData(props, from) {
  ...
  if (!props?.post?.blockMap) {
    // 跳过fallback错误页面（ID不是有效的Notion UUID）
    if (props.post.id === 'oops-page-fallback') {
      props.post.blockMap = { block: {} }
    } else {
      const blockMap = await getPostBlocks(props.post.id, from)
      props.post.blockMap = blockMap?.block ? blockMap : { block: {} }
    }
  }

  if (props.post?.blockMap?.block) {
    // 目录默认加载 - 过滤掉无效的block（防止uuidToId崩溃）
    props.post.content = Object.keys(props.post.blockMap.block).filter(
      key => {
        const block = props.post.blockMap.block[key]
        return block && block.value && block.value.parent_id === props.post.id
      }
    )
  ...
  if (allPosts && allPosts.length > 0) {
    ...
    props.prev = compactPostForArchive(
      allPosts.slice(index - 1, index)[0] ?? allPosts.slice(-1)[0]
    )
    props.next = compactPostForArchive(
      allPosts.slice(index + 1, index + 2)[0] ?? allPosts[0]
    )
    props.recommendPosts = getRecommendPost(props.post, allPosts, siteConfig('POST_RECOMMEND_COUNT')).map(post =>
      compactPostForLatest(post)
    )
  } else {
    props.prev = null
    props.next = null
