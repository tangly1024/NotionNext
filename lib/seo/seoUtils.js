 ...
 * @returns {Array} 优化的关键词列表
 */
export function generateDynamicKeywords(pageData, siteInfo, pageType = 'post') {
  const keywords = []
  const maxKeywords = 8 // 限制关键词数量，避免过长
  ...
  switch (pageType) {
    ...
    case 'article':
      ...
      if (pageData.tags && pageData.tags.length > 0) {
        keywords.push(...pageData.tags.slice(0, 5))
      }
      if (pageData.category) {
        keywords.push(pageData.category)
      }
      // 从标题提取关键词
      if (pageData.title) {
        const titleKeywords = extractKeywordsFromTitle(pageData.title)
    ...
    case 'category':
      // 分类页面关键词
      if (pageData.category) {
        keywords.push(pageData.category, `${pageData.category}文章`, `${pageData.category}内容`)
      }
      if (siteInfo && siteInfo.title) {
        keywords.push(siteInfo.title)
      }
      break
    ...
    case 'tag':
      // 标签页面关键词
      if (pageData.tag) {
        keywords.push(pageData.tag, `${pageData.tag}相关`, `${pageData.tag}文章`)
      }
      if (siteInfo && siteInfo.title) {
        keywords.push(siteInfo.title)
      }
    ...
    case 'search':
      // 搜索页面关键词
      if (pageData.keyword) {
        keywords.push(pageData.keyword, `${pageData.keyword}搜索`, `${pageData.keyword}结果`)
      }
      keywords.push('搜索', '查找')
      if (siteInfo && siteInfo.title) {
        keywords.push(siteInfo.title)
    ...
    default:
      // 首页或网站页面关键词
      if (siteInfo && siteInfo.keywords) {
        const siteKeywords = siteInfo.keywords.split(',').map(k => k.trim())
        keywords.push(...siteKeywords.slice(0, 5))
      }
      if (siteInfo && siteInfo.title) {
        keywords.push(siteInfo.title)
      }
      if (siteInfo && siteInfo.description) {
        const descKeywords = extractKeywordsFromText(siteInfo.description, 3)
...
export function extractKeywordsFromTitle(title) {
  ...
  const stopWords = new Set([
    '的', '了', '在', '是', '我', '有', '和', '就', '不', '人', '都', '一', '一个', '上', '也', '很', '到', '说', '要', '去', '你', '会', '着', '没有', '看', '好', '自己', '这', '那', '个', '为', '与', '或',
    'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'how', 'what', 'why', 'when', 'where', 'who'
  ])
  
  // 提取中文词汇和英文单词
  const words = title.match(/[\u4e00-\u9fa5]{2,}|\b[a-zA-Z]{3,}\b/g) || []
 ...
 * @returns {Array} 关键词列表
 */
export function extractKeywordsFromText(text, maxCount = 5) {
  if (!text) return []
  
  const stopWords = new Set([
    '的', '了', '在', '是', '我', '有', '和', '就', '不', '人', '都', '一', '一个', '上', '也', '很', '到', '说', '要', '去', '你', '会', '着', '没有', '看', '好', '自己', '这', '那', '个', '为', '与', '或', '但', '如果', '因为', '所以', '然后', '现在', '可以', '应该', '需要', '能够', '可能', '已经', '正在', '将要',
    'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'must', 'shall', 'this', 'that', 'these', 'those', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'me', 'him', 'her', 'us', 'them'
  ])
  
  // 提取中文词汇（2-4个字）和英文单词（3个字母以上）
  const words = text.match(/[\u4e00-\u9fa5]{2,4}|\b[a-zA-Z]{3,}\b/g) || []
 ...
 * @returns {string} 格式化的关键词字符串
 */
export function formatKeywordsString(keywords, maxLength = 160) {
  ...
  // 按长度排序，优先使用较短的关键词
  cleanKeywords.sort((a, b) => a.length - b.length)
  
  let result = ''
  for (const keyword of cleanKeywords) {
    const newResult = result ? `${result}, ${keyword}` : keyword
...
export function analyzePostSEO(post, allPosts, siteInfo) {
  return {
    ...
    issues: detectSEOIssues(post, siteInfo),
    recommendations: generateSEORecommendations(post, siteInfo),
    relatedPosts: allPosts ? allPosts.filter(p => 
      p.id !== post.id && 
      p.tags && post.tags && 
