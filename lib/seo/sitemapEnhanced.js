...
function generateMainSitemap(allPages, baseUrl) {
  ...
  allPages?.forEach(post => {
    ...
    // 过滤无效的slug
    if (isValidSlug(slugWithoutLeadingSlash)) {
      urls.push({
        loc: `${baseUrl}/${slugWithoutLeadingSlash}`,
  ...
  // 过滤和清理所有URL
  const cleanUrls = urls.filter(url => isValidUrl(url.loc))

  const xml = createSitemapXml(cleanUrls)
...
function isValidUrl(url) {
  ...
  // 移除包含其他域名的错误URL
  if (url.includes('https://www.netdiskso.xyz') || 
      url.includes('https://shareking.vip/about') ||
      url.includes('https://github.com/') ||
      url.includes('https://tangly1024.com') ||
      url.includes('https://docs.tangly1024.com') ||
      url.includes('https://blog.tangly1024.com')) {
    return false
  }
