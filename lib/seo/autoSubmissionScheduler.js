...
class AutoSubmissionScheduler {
  ...
  async getNewPosts() {
    try {
      // 动态导入NotionNext的数据获取函数
      const { getAllPosts } = await import('../notion/getNotionPost.js')
      ...
      const newPosts = allPosts.filter(post => {
        ...
        const isNew = publishTime > oneDayAgo
        const isPublished = post.status === 'Published'
        
        return isNew && isPublished
      })
      
      // 转换为URL格式
      const baseUrl = siteConfig('LINK') || process.env.NEXT_PUBLIC_SITE_URL || ''
  ...
  async getUpdatedPages() {
    try {
      ...
      const updatedPages = allPages.filter(page => {
        if (!page || !page.lastEditedTime) return false
        
        const lastEditTime = new Date(page.lastEditedTime).getTime()
        const publishTime = page.publishDate ? new Date(page.publishDate).getTime() : 0
        const isUpdated = lastEditTime > oneDayAgo
        const isPublished = page.status === 'Published'
        
        // 确保是更新而不是新发布（避免重复提交）
        const isUpdate = lastEditTime > publishTime + (60 * 1000) // 1分钟容差
        
        return isUpdated && isPublished && isUpdate
      })
      ...
      return updatedPages.map(page => ({
        ...
        changeFreq: page.type === 'Post' ? 'weekly' : 'monthly',
        priority: page.type === 'Post' ? 0.8 : 0.6,
        type: page.type || 'page'
      }))
