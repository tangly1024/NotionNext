...
export function generateSlugLookupJson({ allPages, locale }) {
  const slugLookup = {}

  allPages.forEach(page => {
    if (!page?.id || !page?.slug || page?.status !== 'Published') {
      return
    }

    const normalizedSlug = String(page.slug).replace(/^\/+|\/+$/g, '').toLowerCase()
    if (!normalizedSlug) {
      return
    }

    slugLookup[normalizedSlug] = page.id
  })

  const localeKey = locale || 'zh-CN'
  try {
    fs.writeFileSync(
      `./public/slug-lookup.${localeKey}.json`,
      JSON.stringify(slugLookup)
    )
  } catch (error) {
    console.warn('无法写入 slug lookup 文件', error)
  }
}
