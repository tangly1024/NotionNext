 ...
 * @returns
 */
export function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  // 外链不处理
  ...
  // 直接从 BLOG 对象中获取映射配置，而不是通过 siteConfig 函数
  const POST_URL_PREFIX_MAPPING_CATEGORY = BLOG.POST_URL_PREFIX_MAPPING_CATEGORY || {}

  // 检查是否使用了分类映射模式
  const isCategoryMode = allSlugPatterns.includes('%category%')

  console.log('DEBUG URL Mapping - Category:', postProperties?.category)
  console.log('DEBUG URL Mapping - Patterns:', allSlugPatterns)
  console.log('DEBUG URL Mapping - Is Category Mode:', isCategoryMode)
  console.log(
    'DEBUG URL Mapping - CategoryMap:',
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%' && postProperties?.publishDay) {
      ...
      if (
        Array.isArray(postProperties.category) &&
        postProperties.category.length > 0
      ) {
        categoryPrefix = postProperties.category[0]
        console.log(
          'DEBUG URL Mapping - Category is array, using first category:',
          categoryPrefix
        )
      }
      ...
      if (POST_URL_PREFIX_MAPPING_CATEGORY[categoryPrefix]) {
        const originalCategory = categoryPrefix
        categoryPrefix = POST_URL_PREFIX_MAPPING_CATEGORY[categoryPrefix]
        console.log(
          'DEBUG URL Mapping - Mapped category from',
          originalCategory,
          'to',
          categoryPrefix
        )
        ...
        console.log(
          'DEBUG URL Mapping - No mapping found for category:',
          categoryPrefix
        )
      }

      // 强制使用映射后的分类名，而不是原始的中文分类名
  ...
  // 在分类模式下，我们需要确保使用映射后的分类名
  let finalSlug = '';

  if (fullPrefix) {
    // 如果是分类模式，则fullPrefix已经包含了映射后的分类名
    if (isCategoryMode) {
      finalSlug = `${fullPrefix}/${postProperties.slug ?? postProperties.id}`;
    } else {
      // 其他模式
      finalSlug = `${fullPrefix}/${postProperties.slug ?? postProperties.id}`;
    }
  } else {
    finalSlug = `${postProperties.slug ?? postProperties.id}`;
  }

  console.log('DEBUG URL Mapping - Final URL:', finalSlug);
  return finalSlug
}
