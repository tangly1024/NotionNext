...
function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    // 通用处理 media: video, audio, file, pdf, image
    if (['file', 'pdf', 'video', 'audio', 'image'].includes(b?.value?.type)) {
      let signedUrl = b?.value?.file?.url || b?.value?.format?.display_source

      // 硬兜 attachment:xxx 的情况
      if (!signedUrl && b?.value?.properties?.source?.[0][0]) {
        const rawSource = b?.value?.properties?.source?.[0][0]
        if (rawSource?.startsWith('attachment:')) {
          // 兜 Notion 真正的 signed URL
          signedUrl = `https://www.notion.so/image/${encodeURIComponent(rawSource)}?table=block&id=${b?.value?.id}&cache=v2`
        } else {
          signedUrl = rawSource
        }
      }
      ...
      if (signedUrl) {
        b.value.properties.source = [[signedUrl]]
      }
    }
