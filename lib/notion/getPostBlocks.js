...
function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  normalizeRecordMapWithRoles(clonePageBlock)
  let count = 0
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    // 防御性检查：移除无效的block（防止uuidToId(undefined)崩溃）
    // uuidToId需要block.value.id，所以必须验证其存在性
    if (!blockId || !b || !b.value || !b.value.id) {
      console.warn(`[警告] 移除无效block: blockId=${blockId}, 存在b=${!!b}, 存在value=${!!b?.value}, 存在value.id=${!!b?.value?.id}`)
  ...
  // 最终清理：彻底移除所有无效block，防止uuidToId(undefined)崩溃
  if (clonePageBlock?.block) {
    const blockIds = Object.keys(clonePageBlock.block)
    for (const blockId of blockIds) {
      const block = clonePageBlock.block[blockId]
      // 检查block、block.value、block.value.id都必须存在（uuidToId需要block.value.id）
      if (!block || !block.value || !blockId || !block.value.id) {
        console.warn(`[最终清理] 移除无效block: blockId=${blockId}, hasBlock=${!!block}, hasValue=${!!block?.value}, hasValueId=${!!block?.value?.id}`)
        delete clonePageBlock.block[blockId]
      }
    }
  }
...
function normalizeRecordMapWithRoles(recordMap) {
