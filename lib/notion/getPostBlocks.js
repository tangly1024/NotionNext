...
export async function getPostBlocks(id, from, slice) {
  ...
  if (pageBlock) {
    console.log('[Target cache]:', `from:${from}`, cacheKey)
  ...
  console.log('[API calling time:]', `${end - start}ms`)
...
async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    console.log('[API Request:]', `from:${from}`, `id:${id}`, retryAttempts < 3 ? `Retry :${retryAttempts}` : '')
    try {
      const authToken = BLOG.NOTION_ACCESS_TOKEN || null
      const api = new NotionAPI({ authToken, userTimeZone: 'Asia/ShangHai' })
      const pageData = await api.getPage(id)
      console.info('[Success]:', `from:${from}`, `id:${id}`)
      return pageData
    } catch (e) {
      console.warn('[Abnormal]:', e)
      await delay(1000)
      const cacheKey = 'page_block_' + id
