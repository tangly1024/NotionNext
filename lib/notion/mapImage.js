...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // Notion 图床转换为永久地址
  const hasConverted =
    imgUrl.indexOf('https://www.notion.so/image') === 0 ||
    imgUrl.includes('notion.site/images/page-cover/')
  // 需要转化的URL ; 识别aws图床地址，或者bookmark类型的外链图片
  const needConvert =
    !hasConverted &&
    (block.type === 'bookmark' ||
      imgUrl.includes('secure.notion-static.com') ||
      imgUrl.includes('prod-files-secure'))

  // 使用Notion图传
  if (needConvert) {
    // 检测如果url中包含 &spaceId=xxx 这个参数要去掉，其它保留

    imgUrl = BLOG.NOTION_HOST + '/image/' + encodeURIComponent(imgUrl)
    '?table=' + type + '&id=' + block.id
  }

  if (!isEmoji(imgUrl) && imgUrl.indexOf('notion.so/images/page-cover') < 0) {
    ...
    if (
      imgUrl &&
      imgUrl.length > 4 &&
      !imgUrl.includes('https://www.notion.so/images/')
    ) {
      // 图片接口拼接唯一识别参数，防止请求的图片被缓，而导致随机结果相同
      const separator = imgUrl.includes('?') ? '&' : '?'
      imgUrl = `${imgUrl.trim()}${separator}t=${block.id}`
    }
  }
  ...
  if (needCompress) {
    ...
    imgUrl = compressImage(imgUrl, width)
  }
