...
export function GlobalContextProvider(props) {
  ...
  useEffect(() => {
    ...
    const detectLangFromPath = () => {
      ...
      // Next.js i18n locale 会是 'zh-CN' 或 'en'
      // 如果 router.locale 是 'en'，转换为 'en-US'
      if (router.locale === 'en') {
        return 'en-US'
      }
      // 优先使用 router.locale，否则使用配置的默认语言
      return router.locale || NOTION_CONFIG?.LANG || LANG
    }

    const detectedLang = detectLangFromPath()
    initLocale(detectedLang, changeLang, updateLocale)

    // 如果检测到的语言和当前语言不同，更新语言
