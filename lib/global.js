...
export function GlobalContextProvider(props) {
  ...
  useEffect(() => {
    // 计算距离下一个切换时间点的毫秒数
    const getNextSwitchDelay = (targetHour) => {
      const now = new Date()
      const next = new Date()
    ...
    const setThemeSwitch = () => {
      ...
      // 设置当前主题
      const htmlElement = document.getElementsByTagName('html')[0]
      htmlElement.classList?.remove(shouldBeDark ? 'light' : 'dark')
      htmlElement.classList?.add(shouldBeDark ? 'dark' : 'light')
      saveDarkModeToLocalStorage(shouldBeDark)
      updateDarkMode(shouldBeDark)

      // 设置下一次切换的定时器
      const nextHour = shouldBeDark ? 7 : 19
      const delay = getNextSwitchDelay(nextHour)
      setTimeout(setThemeSwitch, delay)
    }
