...
export default function VoiceChat({ isOpen, onClose }) {
  ...
  const startRecording = () => {
    ...
    rec.onresult = (e) => {
      ...
      if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
      silenceTimerRef.current = setTimeout(() => manualSubmitRecording(), 2200);
    };
    ...
    <div className="fixed inset-0 z-[200] flex flex-col w-full h-[100dvh] bg-slate-950 text-slate-100 overflow-hidden">
      ...
      <div className="relative z-20 flex items-center justify-between px-4 h-16 border-b border-white/10 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setSettings((s) => ({ ...s, showText: !s.showText }))} className={`w-10 h-10 flex items-center justify-center rounded-full active:scale-90 ${settings.showText ? 'bg-pink-500/45 text-white' : 'bg-white/15 text-white'}`} title="字幕开关">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 pb-[max(24px,env(safe-area-inset-bottom))] px-5 bg-gradient-to-t from-slate-950 via-slate-950/92 to-transparent pt-12">
        <div className="max-w-md mx-auto relative flex items-center justify-center">
          <button onClick={() => setTextMode((v) => !v)} className="absolute left-0 w-12 h-12 flex items-center justify-center rounded-full bg-white/15 text-white active:scale-95">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-20 h-20 rounded-full flex items-center justify-center text-white shadow-2xl transition-all duration-300 ${
                    isRecording ? 'bg-red-500 scale-95 ring-4 ring-red-400/40' : 'bg-gradient-to-r from-pink-500 to-rose-500 hover:scale-105 active:scale-95'
                  }`}
                >
                ...
                {isRecording && (
                  <button onClick={manualSubmitRecording} className="h-12 px-4 rounded-xl bg-emerald-500 text-white font-semibold active:scale-95">提交</button>
                )}

                {isRecording && (
                  <button onClick={clearRecordingBuffer} className="h-12 px-3 rounded-xl bg-white/15 text-white active:scale-95">清空</button>
                )}
              </div>
              <div className="text-[10px] font-bold text-slate-300 mt-3 tracking-widest uppercase">长按切换语言 · {currentLangObj.flag} {currentLangObj.name}</div>
            </div>
          ) : (
          ...
          {(isAiSpeaking || isRecording) && !textMode && (
            <button onClick={stopEverything} className="absolute right-0 w-12 h-12 flex items-center justify-center rounded-full bg-white/15 text-white active:scale-95" title="全部停止">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5 no-scrollbar text-slate-200 text-sm">
              ...
              <div className="space-y-4 border-t border-white/10 pt-4">
                ...
                <div className="flex gap-4">
                  <div className="flex-1"><label className="text-xs text-slate-400 block mb-1">语速: {settings.ttsSpeed}x</label><input type="range" min="0.6" max="1.6" step="0.05" className="w-full accent-pink-500" value={settings.ttsSpeed} onChange={(e) => setSettings({ ...settings, ttsSpeed: parseFloat(e.target.value) })} /></div>
                  <div className="flex-1"><label className="text-xs text-slate-400 block mb-1">音调: {settings.ttsPitch}</label><input type="range" min="0.7" max="1.4" step="0.05" className="w-full accent-pink-500" value={settings.ttsPitch} onChange={(e) => setSettings({ ...settings, ttsPitch: parseFloat(e.target.value) })} /></div>
                </div>
              </div>
