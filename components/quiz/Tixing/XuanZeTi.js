...
const isPunctuationOrSymbol = (ch = '') => !isChineseChar(ch) && !isMyanmarChar(ch) && !isLatinOrDigit(ch) && !isWhitespace(ch);

const detectWholeTextType = (text = '') => {
  const hasZh = /[\u4e00-\u9fff]/.test(text); const hasMy = /[\u1000-\u109F]/.test(text); const hasLatin = /[a-zA-Z0-9]/.test(text);
  const count = [hasZh, hasMy, hasLatin].filter(Boolean).length;
  if (count <= 1) { if (hasZh) return 'zh'; if (hasMy) return 'my'; if (hasLatin) return 'en'; }
  return 'mixed';
};
...
function splitMixedText(text = '') {
  ...
  const chars = Array.from(text); const segments = []; let currentText = ''; let currentLang = null;
  const pushCurrent = () => { const t = currentText.trim(); if (t) segments.push({ text: t, lang: currentLang || 'zh' }); currentText = ''; currentLang = null; };
  for (const ch of chars) {
    let lang = null;
    if (isChineseChar(ch)) lang = 'zh'; else if (isMyanmarChar(ch)) lang = 'my'; else if (isLatinOrDigit(ch)) lang = 'en';
    else if (isWhitespace(ch) || isPunctuationOrSymbol(ch)) { currentText += ch; continue; }
    if (!currentLang) { currentLang = lang; currentText += ch; continue; }
    if (lang === currentLang) { currentText += ch; } else { pushCurrent(); currentLang = lang; currentText = ch; }
  }
  pushCurrent();
...
async function getTTSBlob(text, voice, rate = 0, apiUrl = 'https://t.leftsite.cn/tts') {
  ...
  if (!blob) {
    ...
    blob = await res.blob(); await idb.set(cacheKey, blob);
  }
  return blob;
}
...
const audioController = {
  ...
  stop() {
    ...
    if (this.currentAudio) {
      ...
      this.currentAudio.onended = null; this.currentAudio.onerror = null; this.currentAudio = null;
    }
  ...
  async playMixed(text, { rate = 0, aiSettings = null } = {}, onStart, onEnd) {
    ...
    if (!raw) { onEnd?.(); return; }
    const reqId = this.latestRequestId; onStart?.();
    try {
      const segments = splitMixedText(raw); const audios = [];
      for (const seg of segments) {
        if (reqId !== this.latestRequestId) return;
        const segText = String(seg.text || '').trim(); if (!segText) continue;
        const voice = seg.lang === 'my' ? (aiSettings?.myVoice || TTS_VOICES.my) : seg.lang === 'en' ? TTS_VOICES.en : (aiSettings?.zhVoice || TTS_VOICES.zh);
        const blob = await getTTSBlob(segText, voice, rate, aiSettings?.ttsApiUrl || 'https://t.leftsite.cn/tts');
        if (reqId !== this.latestRequestId) return;
        const url = URL.createObjectURL(blob); this.activeUrls.push(url); audios.push(new Audio(url));
      }
      if (reqId !== this.latestRequestId) return;
      if (!audios.length) { onEnd?.(); return; }
      const playNext = (index) => {
        if (reqId !== this.latestRequestId) return;
        if (index >= audios.length) { if (reqId === this.latestRequestId) onEnd?.(); return; }
        const audio = audios[index]; this.currentAudio = audio;
        audio.onended = () => playNext(index + 1); audio.onerror = () => playNext(index + 1);
        audio.play().catch(() => playNext(index + 1));
      };
      playNext(0);
    } catch (e) { console.warn('[TTS Error]:', e); if (reqId === this.latestRequestId) onEnd?.(); }
  }
};
...
.xzt-container.ai-open .result-sheet, .xzt-container.ai-open .submit-bar { visibility: hidden; opacity: 0; pointer-events: none; }
...
.submit-bar { position:absolute; bottom:0; left:0; right:0; padding:16px 16px calc(16px + env(safe-area-inset-bottom)); background:linear-gradient(180deg, rgba(255,255,255,0) 0%, #ffffff 20%); display:flex; justify-content:center; z-index:30; transition: opacity 0.3s; }
...
.result-sheet { position:absolute; bottom:0; left:0; right:0; padding:20px 16px calc(20px + env(safe-area-inset-bottom)); border-top-left-radius:24px; border-top-right-radius:24px; display:flex; flex-direction:column; gap:12px; transform:translateY(100%); transition: transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275), opacity 0.3s; box-shadow:0 -10px 30px rgba(0,0,0,0.08); z-index:100; }
...
/* 模态框底层 - 修改为底部滑出结构 */
.modal-backdrop { position:fixed; inset:0; background:rgba(0,0,0,.4); backdrop-filter:blur(4px); animation: fadeIn 0.3s; }
.panel-modal { position:fixed; inset:0; z-index:2147483600; display:flex; flex-direction:column; justify-content:flex-end; }
.panel-card {
  width:100%; max-height:85vh; background:#fff; border-radius:24px 24px 0 0;
...
.drag-indicator { width: 40px; height: 5px; background: #cbd5e1; border-radius: 99px; margin: 12px auto; }
.panel-header { display:flex; align-items:center; justify-content:space-between; padding:0 20px 16px; border-bottom: 1px solid #f1f5f9; }
.panel-title { font-size:18px; font-weight:900; color:#1e293b; display:flex; align-items:center; gap:8px; }
.panel-close-btn { width:32px; height:32px; border:none; background:#f1f5f9; color:#64748b; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; }
.panel-body { padding: 16px 20px; overflow-y:auto; flex:1; -webkit-overflow-scrolling: touch; }

.settings-section-title { font-size:13px; font-weight:900; color:#94a3b8; margin-bottom:12px; }
.setting-row { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:14px 0; border-bottom: 1px solid #f8fafc; }
.setting-row:last-child { border-bottom: none; }
.setting-label { font-size:15px; font-weight:700; color:#334155; }
.switch { width:50px; height:28px; border-radius:9999px; position:relative; transition:all .2s; cursor:pointer; }
.switch-dot { position:absolute; top:3px; width:22px; height:22px; border-radius:50%; background:#fff; transition:all .2s; box-shadow:0 2px 4px rgba(0,0,0,.2); }
.speed-group { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin-top:10px; }
.speed-btn { border:2px solid #e2e8f0; background:#fff; color:#64748b; border-radius:14px; font-size:14px; font-weight:900; padding:12px 0; cursor:pointer; transition:all 0.1s; }
.speed-btn.active { background:#ecfccb; border-color:#84cc16; color:#4d7c0f; }

.jump-btn { width:100%; background:#f3e8ff; color:#7c3aed; border-radius:16px; padding:16px; font-size:16px; font-weight:900; display:flex; align-items:center; justify-content:space-between; cursor:pointer; border:none; margin-top:20px; }
.jump-btn:active { transform:scale(0.98); }

.panel-footer { padding: 16px 20px calc(16px + env(safe-area-inset-bottom)); border-top: 1px solid #f1f5f9; background:#fff; }
.done-btn { width: 100%; padding: 16px; border-radius: 16px; background: #1e293b; color: #fff; font-size: 16px; font-weight: 900; border: none; cursor: pointer; }
    ...
    <div className="panel-modal">
      ...
      <div className="panel-card relative">
        ...
        <div className="panel-header">
          ...
          <button className="panel-close-btn" onClick={onClose}><FaTimes size={14} /></button>
        </div>

        <div className="panel-body">
          <div className="settings-section-title">显示与朗读</div>
          ...
          <div className="setting-row">
            ...
            >
              <div className="switch-dot" style={{ left: prefs.showQuestionPinyin ? '26px' : '2px' }} />
            </div>
          </div>
          ...
          <div className="setting-row">
            ...
            >
              <div className="switch-dot" style={{ left: prefs.showOptionPinyin ? '26px' : '2px' }} />
            </div>
          </div>
          ...
          <div className="setting-row">
            ...
            >
              <div className="switch-dot" style={{ left: prefs.autoPlay ? '26px' : '2px' }} />
            </div>
          </div>

          <div style={{ marginTop: 24 }}>
            <div className="settings-section-title">题目语速</div>
...
export default function XuanZeTi({ data: rawData, onCorrect, onWrong, onNext }) {
  // ... (主组件的数据处理和逻辑完全保持不变，直到 return 部分)
  ...
  const openOverlay = useCallback((type, setter) => {
    if (typeof window === 'undefined') { setter(true); return; }
  ...
  useEffect(() => { syncOverlayStack('settings', showSettings); }, [showSettings, syncOverlayStack]);
  useEffect(() => { syncOverlayStack('ai-settings', showAISettings); }, [showAISettings, syncOverlayStack]);
  useEffect(() => { syncOverlayStack('ai-explanation', showAIExplanation); }, [showAIExplanation, syncOverlayStack]);

  useEffect(() => {
    mountedRef.current = true;
    return () => { mountedRef.current = false; overlayStackRef.current = []; };
  }, []);

  useEffect(() => { saveInteractivePrefs(prefs); }, [prefs]);
  useEffect(() => { saveInteractiveAISettings(aiSettings); }, [aiSettings]);
  useEffect(() => { setQuestionImgVisible(Boolean(questionImg)); }, [questionImg]);

  const currentRate = speedLabelToRate(prefs.ttsSpeed);

  useEffect(() => {
    clearTimers(); audioController.stop();
    setSelectedIds([]); setIsSubmitted(false); setIsRight(false); setShowResultSheet(false);
    setIsQuestionPlaying(false); setSpeakingOptionId(null); setShowSettings(false); setShowAISettings(false);
    setCardPopId(null); setShowAIExplanation(false); overlayStackRef.current = [];
  ...
  const toggleOption = (id) => {
    ...
    setCardPopId(sid);
    addTimer(() => { if (mountedRef.current) setCardPopId((prev) => (prev === sid ? null : prev)); }, 180);
  ...
  const handleOpenSettings = () => { if (showAIExplanation) return; openOverlay('settings', setShowSettings); };
  const handleOpenAISettings = () => { setShowSettings(false); openOverlay('ai-settings', setShowAISettings); };

  const handleAI = () => {
    audioController.stop(); setIsQuestionPlaying(false); setSpeakingOptionId(null);
    if (!aiSettings.apiKey || !aiSettings.apiUrl || !aiSettings.model) {
      openOverlay('ai-settings', setShowAISettings); return;
    }
    setShowSettings(false); setShowAISettings(false);
  ...
  return (
    <div className={`xzt-container ${hasOverlayOpen ? 'ai-open' : ''}`}>
      ...
      <div className={`result-sheet ${showResultSheet ? 'show' : ''} ${isRight ? 'correct' : 'wrong'}`}>
        <div className="sheet-header">{isRight ? <FaCheck /> : <FaTimes />}<span>{isRight ? '答对了！' : '再试试看'}</span></div>
        <div className="sheet-sub">{isRight ? '太棒了，继续前进。' : '你已经很接近正确答案了。'}</div>
        {!isRight && <button className="ai-btn" onClick={handleAI}><FaRobot /> AI 语音讲题老师</button>}
        <button className={`next-btn ${isRight ? 'btn-correct' : 'btn-wrong'}`} onClick={() => { audioController.stop(); onNext?.(); }}>继续 <FaArrowRight /></button>
      </div>

      {showSettings && (
        <SettingsPanel prefs={prefs} setPrefs={setPrefs} onClose={() => setShowSettings(false)} onOpenAISettings={handleOpenAISettings} />
      )}

      <AISettingsModal open={showAISettings} settings={aiSettings} updateSettings={updateAISettings} onClose={() => setShowAISettings(false)} scene="exercise" />

      <InteractiveAIExplanationPanel
        open={showAIExplanation} onClose={() => setShowAIExplanation(false)} settings={aiSettings} updateSettings={updateAISettings} title="AI 讲题老师"
        initialPayload={{
          questionType: 'choice', questionText, questionImage: questionImg || '',
          options: shuffledOptions.map((opt) => ({ id: String(opt.id), text: opt.text, imageUrl: opt.img || opt.imageUrl || '' })),
          selectedIds: selectedIds.map(String), correctAnswers: correctAnswers.map(String), isRight, extraContext: { multiSelect: correctAnswers.length > 1 }
        }}
      />
    </div>
  );
