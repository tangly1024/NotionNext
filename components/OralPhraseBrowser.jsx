    ...
    >
      ...
      >
        ...
        <div className="flex justify-around items-center px-4 pb-4">
          ...
          >
            <Volume2 size={24} />
            <span className="text-[10px]">整句 | စာကြောင်း</span>
          </div>

          <div
            onClick={toggleRecord}
            className={`w-16 h-16 rounded-full flex items-center justify-center shadow-xl border-4 ${
              recordState === 'recording' ? 'bg-red-500 border-red-100' : 'bg-slate-100'
            }`}
          >
          ...
          <div
            ...
            className={`flex flex-col items-center gap-2 ${
              userAudio ? '' : 'opacity-30'
            }`}
          >
            <Play size={24} />
            <span className="text-[10px]">回放 | ပြန်ဖွင့်ပါ</span>
          </div>
        </div>
      </motion.div>
    </div>
...
export default function OralPhraseBrowser({
  ...
  return (
    // 【修改点】底层背景色和 padding 完全对齐你提供的代码：bg-[#F5F7FA]
    <div className="min-h-screen pb-32 bg-[#F5F7FA] font-sans text-slate-900 max-w-md mx-auto relative shadow-2xl overflow-x-hidden">
      ...
      >
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
          ...
          <span className="text-[10px] text-slate-400 font-burmese">{isFavMode ? 'မှတ်ထားသော စကားပြော' : categoryTitle}</span>
        </div>
      ...
      <div className="pt-20 px-3 space-y-4">
        {displayPhrases.length === 0 && (
          <div className="flex flex-col items-center justify-center pt-32 text-slate-400">
            ...
            {isFavMode && <p className="text-xs font-burmese mt-1">မှတ်ထားသော စာကြောင်း မရှိသေးပါ</p>}
          </div>
        )}
          ...
          return (
            ...
            >
              <div 
                ...
                ${playingId === item.id ? 'ring-2 ring-blue-500 bg-blue-50/10' : ''} active:scale-[0.98] cursor-pointer`}
