...
import { FaArrowLeft, FaBars, FaPaperPlane, FaMicrophone, FaStop, FaKeyboard, FaClosedCaptioning, FaVolumeUp } from 'react-icons/fa';
...
const GlobalStyles = () => (
  <style>{`
    ...
    .can-select { user-select:text; -webkit-user-select:text; }
    ...
    .animate-ripple-1{animation:ripple-out 2s cubic-bezier(0,0.2,0.8,1) infinite;}
    .animate-ripple-2{animation:ripple-out 2s cubic-bezier(0,0.2,0.8,1) infinite 1s;}
...
const PinyinText = React.memo(({ text='', showPinyin=false, isStreaming=false }) => {
  ...
  return (
    <div className="flex flex-wrap items-end text-[17px] text-slate-800 font-medium tracking-wide gap-y-8" style={{ lineHeight: '3.2' }}>
      {tokens.map((t,idx)=>(
        <div key={`${idx}-${t.text}`} className={`flex flex-col items-center justify-end ${t.isZh?'mx-[1.5px]':''}`}>
          {showPinyin && t.isZh && (
            <span className="text-[12px] leading-none text-blue-500 mb-[4px] font-normal tracking-tight" style={{ fontFamily: 'Arial, sans-serif' }}>{t.py}</span>
          )}
...
function HeaderBar({ title, onBack, onOpenSettings }) {
  return (
    <div className="flex-none flex h-[60px] items-center justify-between px-4 bg-[#f8fafc]/95 backdrop-blur-md border-b border-slate-200/50 z-20">
      <button onClick={onBack} className="flex h-10 w-10 items-center justify-center text-slate-500 hover:text-slate-800 active:scale-90 transition-transform"><FaArrowLeft size={18}/></button>
      <div className="text-[15px] font-black tracking-widest text-slate-800">{title}</div>
      <button onClick={onOpenSettings} className="flex h-10 w-10 items-center justify-center text-slate-500 hover:text-slate-800 active:scale-90 transition-transform"><FaBars size={18}/></button>
    </div>
  );
}
            ...
            <div key={msg.id} className="mb-14 w-full flex flex-col items-start">
              ...
              {!msg.isStreaming && aiText && (
                <div className="flex items-center gap-4 mt-4 pl-1">
                  ...
                  >{isAiSpeaking?<FaStop size={14}/>:<FaVolumeUp size={16}/>}</button>
                  <button 
...
function BottomControlBar({ textMode, setTextMode, inputText, setInputText, sendMessage, isRecording, isAiSpeaking, isThinking, stopEverything, handleMicPointerDown, handleMicPointerUp, handleMicPointerCancel, showText, setShowText }) {
  const LANGUAGES=[
    {code:'zh',label:'中文',flag:'🇨🇳'},{code:'en',label:'English',flag:'🇺🇸'},{code:'ja',label:'日本語',flag:'🇯🇵'},
    {code:'ko',label:'한국어',flag:'🇰🇷'},{code:'th',label:'ไทย',flag:'🇹🇭'},{code:'vi',label:'Tiếng Việt',flag:'🇻🇳'},
    {code:'fr',label:'Français',flag:'🇫🇷'},{code:'es',label:'Español',flag:'🇪🇸'},{code:'ru',label:'Русский',flag:'🇷🇺'},
  ...
  const handleInput = (e)=>{ setInputText(e.target.value); if(textareaRef.current){textareaRef.current.style.height='auto'; textareaRef.current.style.height=`${textareaRef.current.scrollHeight}px`; } };
  const handleSend = ()=>{ if(inputText.trim()){ sendMessage(inputText); setInputText(''); if(textareaRef.current) textareaRef.current.style.height='auto'; } };

  const handleMicDown=(e)=>{
    if(longPressTimer.current) clearTimeout(longPressTimer.current);
    longPressTimer.current = setTimeout(()=>{ setShowLangMenu(true); if(window.navigator.vibrate) window.navigator.vibrate(50); },350);
    handleMicPointerDown(e,currentLang.code);
  };
  const handleMicUp=(e)=>{ if(longPressTimer.current) clearTimeout(longPressTimer.current); handleMicPointerUp(e); };
  const handleMicCancel=(e)=>{ if(longPressTimer.current) clearTimeout(longPressTimer.current); handleMicPointerCancel(e); };
  const selectLang=(e,lang)=>{ e.preventDefault(); e.stopPropagation(); setCurrentLang(lang); setShowLangMenu(false); };

  return (
    <div className="flex-none bg-[#f8fafc] px-5 pt-3 pb-[calc(env(safe-area-inset-bottom+60px))] z-30">
      <div className="flex items-end justify-center gap-5 max-w-lg mx-auto relative">
        <button onClick={()=>setTextMode(!textMode)} className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-white border border-slate-200 text-slate-500 hover:text-slate-700 active:scale-95 transition shadow-sm mb-[2px]">{textMode?<FaMicrophone size={20}/>:<FaKeyboard size={20}/>}</button>
        <div className="flex-1 flex justify-center w-full max-w-[280px]">
          {textMode ? (
            <div className="w-full flex items-end border-[1.5px] border-slate-200 rounded-2xl bg-white shadow-sm overflow-hidden min-h-[48px] focus-within:border-pink-300 focus-within:shadow-md transition-all">
              <textarea ref={textareaRef} value={inputText} onChange={handleInput} placeholder="继续问老师..." className="flex-1 max-h-32 resize-none py-[13px] pl-4 pr-2 text-slate-800 text-[15px] outline-none bg-transparent no-scrollbar leading-snug" rows={1} onKeyDown={e=>{ if(e.key==='Enter'&&!e.shiftKey){ e.preventDefault(); handleSend(); }}}/>
              <button onClick={handleSend} className="p-3 mb-[2px] text-pink-500 hover:text-pink-600"><FaPaperPlane size={18}/></button>
            </div>
          ):(
            <div className="relative flex justify-center pb-[2px]">
              <button onPointerDown={handleMicDown} onPointerUp={handleMicUp} onPointerCancel={handleMicCancel} onPointerLeave={handleMicCancel} onContextMenu={e=>e.preventDefault()} className={`touch-none h-20 w-20 rounded-full text-white flex items-center justify-center transition-all duration-300 shadow-md ${isRecording?'bg-pink-500 scale-110 shadow-[0_4px_20px_rgba(236,72,153,0.5)] animate-pulse-ring':isAiSpeaking||isThinking?'bg-pink-300':'bg-pink-500'}`}>{isRecording?<FaStop size={30}/>:<FaMicrophone size={32}/>}</button>
              ...
              {showLangMenu && (
                <>
                  <div className="fixed inset-0 z-[50]" onPointerDown={()=>setShowLangMenu(false)}/>
                  <div className="absolute bottom-[90px] left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-xl border border-slate-200 shadow-[0_10px_40px_rgba(0,0,0,0.15)] rounded-2xl p-2 w-[260px] max-h-[300px] overflow-y-auto z-[60] no-scrollbar">
                    <div className="sticky top-0 bg-white/95 backdrop-blur-sm z-10 text-center text-xs text-slate-400 font-bold pb-2 pt-1 border-b border-slate-100 mb-1">选择识别语言</div>
                    <div className="grid grid-cols-2 gap-1">
                      {LANGUAGES.map(lang=><button key={lang.code} onPointerDown={e=>selectLang(e,lang)} className="flex items-center justify-between px-4 py-3.5 rounded-xl hover:bg-slate-100 active:bg-slate-200 transition"><span className="text-[15px] font-medium text-slate-700">{lang.label}</span><span className="text-xl">{lang.flag}</span></button>)}
                    </div>
                  </div>
                </>
              )}
        ...
        <button onClick={()=>setShowText(!showText)} className={`relative flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl transition-all shadow-sm mb-[2px] border ${showText ? 'bg-pink-50 border-pink-300 text-pink-500' : 'bg-white border-slate-200 text-slate-400'}`}
        >
