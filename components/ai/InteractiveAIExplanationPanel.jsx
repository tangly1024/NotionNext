            ...
            return (
              ...
              >
                <div className="flex flex-col items-center justify-center gap-2">
                  <span className="pointer-events-none text-2xl">{lang.flag}</span>
                  <span className="pointer-events-none text-xs font-bold text-slate-600">
                    {lang.name}
                  </span>
                </div>
              </button>
...
export default function InteractiveAIExplanationPanel({
  ...
  // 修复点：聊天区显示只由 showText 控制，不再和 textMode 绑死
  const shouldShowConversation = showText;

  return (
    <div className="fixed inset-0 z-[2147483000] flex h-[100dvh] w-full flex-col overflow-hidden bg-white text-slate-800">
      <GlobalStyles />
      ...
      {/* Header */}
      <div className="relative z-20 flex h-16 items-center justify-between border-b border-slate-200/70 px-4 backdrop-blur-sm">
        <button
          type="button"
          onClick={() => {
            stopEverything();
      ...
      <div
        ref={scrollRef}
        className={`relative z-10 flex-1 overflow-y-auto p-4 pb-40 overscroll-contain ${
      ...
      >
        {!shouldShowConversation ? (
          <div className="flex min-h-full flex-1 flex-col items-center justify-center">
            <div className="relative flex h-56 w-56 items-center justify-center pointer-events-none">
              {isAiSpeaking && (
          ...
          <div className="mx-auto mt-auto flex w-full max-w-2xl flex-col justify-end">
            {history.map((msg) => {
              if (msg.role === 'error') {
                return (
      ...
      {/* Bottom Control */}
      <div className="pointer-events-auto absolute bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-white via-white/95 to-transparent px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-12">
