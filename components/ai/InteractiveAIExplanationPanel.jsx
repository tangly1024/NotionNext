...
function HeaderBar({ title, onBack, onOpenSettings }) {
  return (
    <div className="flex-none flex h-[60px] items-center justify-between px-4 bg-[#f8fafc]/95 backdrop-blur-md border-b border-slate-200/50 z-20">
      ...
      <button onClick={onOpenSettings} className="flex h-10 w-10 items-center justify-center text-slate-400 hover:text-slate-800 active:scale-90 transition-transform"><FaSlidersH size={18} /></button>
    </div>
  );
}
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-col pb-8">
        {history.map(msg=>{
          ...
          if(msg.role==='user') return (
            <div key={msg.id} className="mb-8 w-full text-left pl-2">
              <div className="text-[15px] font-medium leading-[1.8] text-slate-400 tracking-wide">{msg.text}</div>
            </div>
          );
            ...
            <div key={msg.id} className="mb-14 w-full flex flex-col items-start">
              ...
              {!msg.isStreaming && aiText && (
                <div className="flex items-center gap-5 mt-4 pl-1">
                  <button onClick={()=>replaySpecificAnswer(msg.text)} className="text-slate-300 hover:text-pink-500 transition-colors p-1" title="朗读"><FaVolumeUp size={20}/></button>
        ...
        {isRecording && textMode===false && inputText && (
          <div className="mb-6 w-full text-left pl-2">
            <div className="text-[15px] font-medium text-pink-400 animate-pulse tracking-wide leading-relaxed">{inputText}</div>
          </div>
        )}
      </div>
    </div>
...
function BottomControlBar({ textMode, setTextMode, isRecording, isThinking, isAiSpeaking, inputText, setInputText, sendMessage, stopEverything, handleMicPointerDown, handleMicPointerUp, handleMicPointerCancel, showText, setShowText }) {
  ...
  return (
    <div className="flex-none bg-gradient-to-t from-[#f8fafc] via-[#f8fafc]/95 to-transparent px-4 pt-6 pb-[calc(env(safe-area-inset-bottom)+16px)] z-30">
      <div className="mx-auto w-full max-w-lg flex items-center justify-between gap-3 relative">
        <button onClick={()=>setTextMode(!textMode)} className="flex h-[56px] w-[72px] items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-slate-600 active:scale-95 shadow-sm">{textMode?'⌨️':'🎤'}</button>
        {textMode?(
          <div className="flex-1 flex items-center bg-white border border-slate-200 rounded-xl h-[64px] pl-5 pr-3 shadow-sm">
            <input type="text" className="flex-1 bg-transparent text-[17px] text-slate-800 outline-none placeholder-slate-300 h-full" placeholder="打字提问..." value={inputText} onChange={e=>setInputText(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&inputText.trim()){sendMessage(inputText); setInputText('');}}}/>
            <button onClick={()=>{if(inputText.trim()){sendMessage(inputText);setInputText('');}}} className={`flex shrink-0 h-[50px] w-[70px] items-center justify-center rounded-lg transition-all active:scale-90 ${inputText.trim()?'bg-pink-500 text-white':'bg-slate-100 text-slate-300'}`}><FaPaperPlane size={14}/></button>
          </div>
        ):(
          <div className="flex-1 flex justify-center relative">
            <button 
              ...
              className={`flex h-[72px] w-[140px] items-center justify-center rounded-xl text-white transition-all duration-300 ${
                isRecording?'bg-red-500 scale-110 animate-pulse-ring':isAiSpeaking||isThinking?'bg-pink-300':'bg-pink-500 shadow-[0_8px_20px_rgba(236,72,153,.35)] hover:scale-105 active:scale-95'
              }`}
            >
              {isRecording?<FaStop size={28}/>:<FaMicrophone size={28}/>}
            </button>
            {isRecording && <div className="absolute -bottom-10 flex gap-2 text-sm font-bold text-red-500 animate-pulse"><span>{currentLang.flag}</span><span>{currentLang.label}</span></div>}
            {showLangMenu && <div className="absolute bottom-[90px] bg-white border border-slate-200 shadow-lg rounded-xl p-3 grid grid-cols-2 gap-2">
              {LANGUAGES.map(lang=>(
        ...
        <button onClick={()=>setShowText(!showText)} className={`shrink-0 flex h-[46px] w-[46px] items-center justify-center rounded-full transition-all active:scale-90 shadow-sm border ${showText?'bg-pink-500 border-pink-500 text-white':'bg-white border-slate-200 text-slate-400'}`}>{showText?<FaClosedCaptioning size={20}/>:<FaCommentSlash size={20}/>}</button>
      </div>
    </div>
  );
}
