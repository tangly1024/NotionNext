...
export const shouldHideLine = (line) => {
  ...
  if (/(拼音|罗马音|romanization|pinyin)/i.test(text)) return true;

  const hasZhOrMy = /[\u4e00-\u9fff\u1000-\u109F]/.test(text);
  const hasLatin = /[a-zA-Z]/.test(text);
  if (hasLatin && !hasZhOrMy) return true;
  if (!hasZhOrMy && EN_ONLY_LINE_RE.test(text)) return true;
  return false;
};
...
export const splitSpeakable = (buf = '') => {
  ...
  for (let i = 0; i < buf.length; i += 1) {
    if (/[。！？.!?\n，,；;]/.test(buf[i])) {
      const sentence = buf.slice(last, i + 1).trim();
