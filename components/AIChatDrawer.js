...
import { 
  saveToUserDict, matchFromUserDict, getAllUserDict, 
  deleteUserDictEntry, clearUserDict 
} from '@/lib/userDict';
...
// ----------------- Helpers -----------------
const safeLocalStorageGet = (key) => { try { return typeof window !== 'undefined' ? localStorage.getItem(key) : null; } catch(e){ return null; } };
const safeLocalStorageSet = (key, value) => { try { if (typeof window !== 'undefined') localStorage.setItem(key, value); } catch(e){} };
const nowId = () => `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
const playBeep = () => { try { const ctx = new (window.AudioContext || window.webkitAudioContext)(); const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.frequency.value = 600; gain.gain.value = 0.08; osc.start(); setTimeout(() => { osc.stop(); ctx.close(); }, 120); } catch (e) {} };
...
const compressImage = (file) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      const img = new Image(); img.src = e.target.result; img.onerror = reject;
      img.onload = () => {
        const canvas = document.createElement('canvas'); const MAX_WIDTH = 1280;
...
const saveLastSuccessModel = (provider, model) => {
  safeLocalStorageSet(getLastSuccessModelKey(provider), model);
};

const clearLastSuccessModel = (provider, model) => {
  const saved = safeLocalStorageGet(getLastSuccessModelKey(provider));
  if (saved === model) {
    safeLocalStorageSet(getLastSuccessModelKey(provider), '');
  }
};
...
const buildSystemInstruction = (enableBackTranslation, tgtLangName, customPrompt) => {
...
2. PURE OUTPUT: The 'translation' field MUST contain ONLY the final translated text in ${tgtLangName}. DO NOT append original texts, phonetic transcriptions, or explanations.
...
4. Return ONLY valid JSON.`;
  ...
  if (!enableBackTranslation) return `${baseRules}\n\nOutput schema:\n{"data":[{"translation":"..."}]}`;
...
const normalizeTranslations = (raw, enableBackTranslation = false) => {
  try {
    ...
    const valid = data.filter(i => i && i.translation).map(i => ({ translation: String(i.translation).trim(), back_translation: enableBackTranslation ? String(i.back_translation||'').trim() : '' }));
...
const TranslationCard = memo(({ data, onPlay, originalText, srcLang, tgtLang }) => {
  ...
  const handleEdit = async (e) => {
    e.stopPropagation();
    const newTrans = prompt("修改并保存到用户词典:", data.translation);
    if (newTrans && newTrans.trim() !== data.translation) {
      await saveToUserDict(srcLang, tgtLang, originalText, newTrans);
...
const SettingsModal = ({ settings, onSave, onClose }) => {
  ...
  const [tab, setTab] = useState('api'); // api, dict, common
        ...
        <Dialog.Panel className="w-full max-w-lg bg-white rounded-3xl shadow-2xl flex flex-col max-h-[85vh]">
          <div className="flex bg-gray-50/50 rounded-t-3xl border-b">
            {['api', 'dict', 'common'].map(t => (
              <button key={t} onClick={()=>setTab(t)} className={`flex-1 py-4 text-sm font-bold ${tab===t ? 'text-pink-600 bg-white rounded-tl-3xl shadow-sm':'text-gray-500'}`}>
          ...
          <div className="p-6 overflow-y-auto slim-scrollbar flex-1">
            ...
            {tab === 'dict' && (
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-xl p-2 min-h-[200px] max-h-[300px] overflow-y-auto">
                  {dictList.length === 0 ? <p className="text-center text-gray-400 mt-10 text-sm">暂无自定义词条</p> : 
                    dictList.map(d => (
            ...
            {tab === 'common' && (
              <div className="space-y-5">
                ...
                <div><span className="text-sm font-bold flex justify-between"><span>语音静默发送</span><span>{data.voiceAutoSendDelay/1000}s</span></span><input type="range" min="1000" max="3000" step="100" value={data.voiceAutoSendDelay} onChange={e=>setData({...data, voiceAutoSendDelay:e.target.value})} className="w-full accent-pink-500 mt-2"/></div>
              </div>
            )}
          </div>
...
const AiChatContent = ({ onClose }) => {
  ...
  const scrollRef = useRef(null); const recognitionRef = useRef(null);
  const abortControllerRef = useRef(null); const currentRequestIdRef = useRef(null);
  const finalTextRef = useRef(''); const autoSendTimerRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (s) {
      try {
        ...
        if (parsed.apiKey && !parsed.apiKeys) { parsed.apiKeys = {[parsed.provider||DEFAULT_PROVIDER]: parsed.apiKey}; delete parsed.apiKey; }
  ...
  useEffect(() => { safeLocalStorageSet('ai886_settings', JSON.stringify({...settings, lastSourceLang: sourceLang, lastTargetLang: targetLang})); }, [settings, sourceLang, targetLang]);
  ...
  const fetchAi = async ({ messages, signal }) => {
    ...
    for (let currentModel of orderedModels) {
      try {
        ...
        const res = await fetch(endpoint, { method: 'POST', headers, body: JSON.stringify({ model: currentModel, messages, temperature: 0, response_format: { type: 'json_object' } }), signal });
        ...
        lastErr = e.message; clearLastSuccessModel(config.provider, currentModel);
  ...
  const handleTranslate = async (textOverride = null) => {
    ...
    setIsLoading(true);
    setHistory(p => [...p, { id: reqId+'_u', role: 'user', text, images: inputImages, ts: Date.now() }]);
    setInputVal(''); finalTextRef.current = ''; setInputImages([]); scrollToResult();

    const sysMsg = buildSystemInstruction(settings.enableBackTranslation, getLangName(cTgt), settings.customPrompt);
...
if (inputImages.length === 0 && text) {
  try {
    ...
    if (dict) {
      ...
      if (dictHit) {
        providerInfo = { name: '★ 离线专业词库', icon: 'fa-book' };
      }
    }
  } catch(e) {}
}
...
if (!dictHit) {
  ...
  if (dictHit) {
    providerInfo = { name: '✓ 我的词典', icon: 'fa-user-edit' };
  }
}
...
if (dictHit && dictHit.length > 0 && dictHit[0].translation !== '（字典数据为空）') {
  ...
  // 成功后静默自动保存到用户词典
  if (text && aiMsg.results[0]?.translation) {
    saveToUserDict(cSrc, cTgt, text, aiMsg.results[0].translation).catch(() => {});
  }
              }

      if (currentRequestIdRef.current === reqId) {
        setHistory(p => [...p, aiMsg]); scrollToResult();
        if (settings.autoPlayTTS && aiMsg.results[0]) playTTS(aiMsg.results[0].translation, cTgt, settings);
      }
    } catch (e) {
      if (e.name !== 'AbortError' && currentRequestIdRef.current === reqId) setHistory(p => [...p, { id: reqId+'_e', role: 'error', text: e.message }]);
    } finally {
  ...
  const startRecording = () => {
    ...
    if (isRecording) { recognitionRef.current?.stop(); if(finalTextRef.current||inputVal) handleTranslate(finalTextRef.current||inputVal); return; }
    playBeep(); const rec = new SR(); rec.lang = sourceLang; rec.interimResults = true; rec.continuous = true;
    ...
    rec.onresult = (e) => {
      ...
      if (autoSendTimerRef.current) clearTimeout(autoSendTimerRef.current);
      autoSendTimerRef.current = setTimeout(() => { if(finalTextRef.current.trim()){ rec.stop(); handleTranslate(finalTextRef.current); } }, settings.voiceAutoSendDelay);
    };
    rec.onend = () => { setIsRecording(false); if (autoSendTimerRef.current && finalTextRef.current.trim()) handleTranslate(finalTextRef.current); };
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar relative z-10 px-4 pt-4 pb-[180px] scroll-smooth">
        <div className="w-full max-w-[600px] mx-auto min-h-full flex flex-col justify-end">
           {history.length === 0 && !isLoading && (
             <div className="text-center text-gray-400 mb-20 opacity-80 mt-20"><div className="w-20 h-20 bg-pink-50 rounded-full flex items-center justify-center mx-auto mb-4 text-4xl">🌍</div><div className="font-bold">极速翻译引擎</div></div>
           )}
           {history.map((item) => (
           ...
           {isLoading && <div className="flex justify-start mb-6"><div className="bg-white/90 px-5 py-3 rounded-2xl shadow-sm text-pink-500 font-bold text-sm"><i className="fas fa-circle-notch fa-spin mr-2" />解码中...</div></div>}
        </div>
      </div>
