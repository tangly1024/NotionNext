...
function AiChatContent() {
  ...
  // ====== 新增：实时文本语言自适应切换 ======
  useEffect(() => {
    if (isRecording) return; // 语音输入期间不自动切换，防止中断当前录音设定

    const text = inputVal.trim();
    if (!text) return;

    // 增加 400ms 防抖，避免打字混合中文英文过程中频繁闪烁
    const timer = setTimeout(() => {
      const detected = detectScript(text);
      if (detected && detected !== sourceLang) {
        if (detected === targetLang) {
          // 如果打字的语言正是目标语言，那么直接自动互换
          swapLanguages();
        } else {
          // 如果是第三方语言，直接把源语言切换为该语言
          setSourceWithPersist(detected);
        }
      }
    }, 400);

    return () => clearTimeout(timer);
  }, [inputVal, isRecording, sourceLang, targetLang, swapLanguages, setSourceWithPersist]);
  // ===========================================
