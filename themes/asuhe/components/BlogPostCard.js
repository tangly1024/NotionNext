    ...
    >
      <div className="lg:py-8 py-4 flex flex-col w-full">
        ...
        >
          <div>
            {CONFIG.POST_LIST_COVER && (
              <div className="w-full max-h-96 object-cover overflow-hidden mb-2 relative rounded-t-2xl">
                <LazyImage
                  src={
                    post?.pageCoverThumbnail ||
                    randomElement(CONFIG.DEFAULT_POST_IMAGE)
                  }
                  className="w-full max-h-96 object-cover hover:scale-125 duration-150"
                />
                <Waves style={wavesStyle} />
              </div>
            )}
            {post.title}
          </div>
        ...
        <div className="flex mt-2 items-center justify-start flex-wrap space-x-3 text-gray-400">
          ...
          {CONFIG.POST_LIST_CATEGORY && (
            <CategoryItem category={post.category} />
          )}
          {CONFIG.POST_LIST_TAG &&
