    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setHotList(data.data); // 修改 setHotList
    ...
    return (
        <Card className='bg-white dark:bg-[#1e1e1e]  dark:border-gray-700 rounded-xl max-h-[450px] overflow-hidden'>
            <div className='flex items-center p-4 border-b dark:border-gray-700'>
                ...
                <h2 className="text-xl font-bold text-gray-700 dark:text-white">抖音热搜榜</h2>
            </div>
                ...
                <ul className="relative" ref={scrollRef}>
                    {hotList.map((item, index) => (
                       <li key={index} className="py-2 border-b dark:border-gray-700 " style={{ whiteSpace: 'nowrap' }}>
                           <a href={item.vieo_link} target="_blank" rel="noopener noreferrer" className="hover:text-indigo-600 dark:hover:text-yellow-600  line-clamp-2 text-gray-700 dark:text-gray-200">
                               <span className="text-gray-500  mr-2 dark:text-gray-500">{item.position}.</span>
                                 {item.word}
                           </a>
                        </li>
                    ))}
