...
export default function ArticleAdjacent({ prev, next }) {
  if (!prev || !next || !CONFIG_APPLE_IOS_HEXO.ARTICLE_ADJACENT) {
    return <></>
  }
  return (
    <section className="text-gray-800 items-center text-xs md:text-sm flex justify-between m-1 ">
      ...
      <Link href={`/article/${next.slug}`} passHref>
        <a className="py-1 cursor-pointer hover:underline justify-end items-center dark:text-white flex w-full h-full duration-200">
          {next.title}
