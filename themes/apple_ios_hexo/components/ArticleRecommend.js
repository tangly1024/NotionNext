...
export default function ArticleRecommend({ recommendPosts }) {
  if (
    !CONFIG_APPLE_IOS_HEXO.ARTICLE_RECOMMEND ||
    !recommendPosts ||
  ...
  return (
    <div className="p-2">
      <div className="font-sans px-1 flex flex-nowrap justify-between dark:text-gray-400 my-4">
        <div>
        ...
        {recommendPosts.map(post => {
          ...
          return (
            ...
            >
              <a
                key={post.id}
                className="flex h-40 cursor-pointer overflow-hidden rounded-xl hover:scale-110 transform duration-200"
              >
                <div
                ...
                >
                  <div className="flex items-center justify-center bg-black bg-opacity-60 hover:bg-opacity-10 w-full h-full duration-300 ">
                    <div className="font-sans text-sm  text-white text-center shadow-text mx-4">
                      <div>
