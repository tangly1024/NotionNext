...
function SiteInfo({ title }) {
  ...
  const cdnInfo = (async () => {
    // try {
    //     let response = await axios.get("/cdn-cgi/trace");
    ...
    //     let area = response.data.split("colo=")[1].split("\n")[0];
    //     document.getElementById("user_ip").innerHTML = "访客IP:"+response.data.split("ip=")[1].split("\n")[0];
    //     for (var i = 0; i < areas.length; i++) {
    //         if (areas[i].indexOf(area) != -1) {
    //             document.getElementById("cf_cdn").innerHTML = "当前CDN节点:"+areas[i];
    //         }
    //     }
    ...
    try {
      ...
      //             "org": "AS138456 OceanBlue Cloud Technology Limited",
  ...
  return (
    ...
    >
      <span> © {`${copyrightDate}`} <span> <a href={BLOG.LINK}> <i className='mx-1 animate-pulse fas fa-heart' /> {BLOG.AUTHOR}</a>. <br /></span>
        ...
        <span id="user_ip">访客IP:unknown</span> <br />
