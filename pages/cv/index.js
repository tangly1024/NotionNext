...
/**
 * /cv/profile
 * - 使用 DynamicLayout 主题容器（layoutName 可按你的主题改成 'LayoutArchive' 或 'LayoutPage'）
 * - 支持 URL Hash 平滑滚动（如 /cv/profile#education）
 * - 内容来自本地结构化数据（后续可替换为 Notion/CMS）
 */
const ProfilePage = (props) => {
  // 与 ArchiveIndex 一样的锚点平滑滚动
  useEffect(() => {
    if (isBrowser) {
      ...
      <main className='max-w-4xl mx-auto px-4 md:px-6 lg:px-8 my-8'>
        ...
        <header className='my-6'>
          ...
          <nav className='flex flex-wrap gap-3 mt-4 text-sm'>
            <a href="#about" className='px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'>About</a>
            <a href="#research" className='px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'>Research</a>
            <a href="#education" className='px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'>Education</a>
            <a href="#work" className='px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'>Work</a>
            <a href="#service" className='px-2 py-1 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800'>Service</a>
          </nav>
        </header>
...
/** —— 数据（可替换为 props.profile 或 Notion 数据） —— */
...
/** SSG + ISR：不依赖外部数据也能构建成功 */
export async function getStaticProps ({ locale }) {
  const props = await getGlobalData({ from: 'cv-profile', locale })
  return {
    props,
