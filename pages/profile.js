...
function getProfileData() {
  return {
    ...
    education: [
      {
        ...
        dissertation: {
          title:
            'Enhancing human-robot collaboration through intuitive Augmented Reality interface and precise visualization',
          url:
            'https://www.proquest.com/docview/2908918372?Theses&fromopenview=true&pq-origsite=gscholar&sourcetype=Dissertations'
        }
      },
      {
        degree: 'M.S.',
    ...
    service: {
      ...
      committees: ['ASME CIE — Virtual Environments Systems (VES) | 2024 – 2025']
    }
  }
}
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Assistant Professor at Lamar University | XR² Lab | Human-Robot Interaction & AR/VR"
        />
      </Head>
        ...
        <main className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 my-8">
          <header className="my-6">
            <h1 className="text-3xl font-bold">
              Dr. Wenhao Yang
              <span className="ml-2 text-sm px-2 py-1 rounded bg-gray-200 dark:bg-gray-700">
                XR² Lab
              </span>
            </h1>
...
export async function getStaticProps({ locale }) {
  const globalProps = await getGlobalData({ from: 'profile', locale })

  // 关键：把前端数据注入 props
  globalProps.profile = getProfileData()

  return {
    props: globalProps,
    revalidate: process.env.EXPORT
      ? undefined
      : siteConfig('NEXT_REVALIDATE_SECOND', BLOG.NEXT_REVALIDATE_SECOND, globalProps.NOTION_CONFIG)
  }
}
